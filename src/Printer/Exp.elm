...
print exp =
    case exp of
        ...
        EConst [] (EVar [] "main") -> ""
        EConst ["const", spc] e -> "const" ++ spc ++ print e
        EConst [] e -> "const " ++ print e
        EGList ws e1 e2 -> 
            case ws of
                [spc1, spc2] ->
                    "{" ++ spc1 ++ print e1 ++ printList e2 ++ "}" ++ spc2
                ...
                _ ->
                    "Error 23"
        EDelta [spc] e delta -> "("++ print e ++")_("++  printDeltaV delta ++")" ++ spc
        _ -> "failed: " ++ toString exp


printDeltaV : Delta -> String 
printDeltaV delta = 
    case delta of 
        DId  ->  ""
        (DReplV (VFloat n)) -> "repl " ++ toString n
        (DAddV (VFloat n)) -> "+" ++ toString n
        DDeleteV -> "del" 
        DInsertV -> "ins"
        (DCom d1 d2) -> printDeltaV d1 ++ " . " ++ printDeltaV d2
        _        -> "fail: unsupported delta value"
