...
print v  =
    case v of
        ...
        VDelta v1 d -> print v1 ++ ("<>") ++ printDelta d

printBV : BValue -> String
printBV v  =
    case v of
        BTrue            -> "true"
        BFalse           -> "false"
        BTuple v1 v2     -> "( "++(printBV v1)++", "++(printBV v2)++" )"
        GTuple v1 v2     -> "{ "++(printBV v1)++", "++(printBV v2)++" }"
        BFix env e       -> "<<< " ++ printBEnv env ++ ", " ++ "Fix " ++ Exp.print e ++ " >>> "
        BClosure env p e -> "<<< " ++ printBEnv env ++ ", " ++ "λ " ++ printPattern p ++ ". " ++ Exp.print e ++ " >>> "
        BError           -> "error"
...
printBEnv : BEnv -> String
printBEnv env =
    "{ " ++ (List.take (List.length env - 7) env |> printBEnv_) ++ "}"
...
printDelta delta =
    case delta of
        DId  -> "id "
        DReplV v -> "repl " ++ print v
        DAddV v -> "+ " ++ print v
        DDeleteV -> "del"
        DInsertV -> "inst"
        DMulV v d -> print v ++ "*" ++ printDelta d
        DAdd p ->
            "+ " ++ printParam p ++ " "
...
printParam param =
    case param of
        ...
        AGCons p1 p2      -> printParam p1 ++ " :: " ++ printParam p2
