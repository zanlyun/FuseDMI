...
operators =
    ...
    , [Infix gcons AssocRight] -- Order matters, shall before cons
...
gListLoop : Parser Exp
gListLoop = 
    loop [] listHelper |> (map exprListToEGList)
...
exprListToEGList : List (Exp, WS) -> Exp
exprListToEGList ls =
    case ls of
        [] ->
            EEmpList []
        (e, ws) :: ts ->
            EGList ws e (exprListToEGList ts)
...
gcons : Parser (Exp -> Exp -> Exp)
gcons =
    succeed (\spc -> EGCons [spc])
        |. symbol ":::"
        |= mspaces
...
-- Const
uConstParser : Parser (Exp -> Exp)
uConstParser  = 
    succeed (\spc1 -> EConst ["const", spc1] )
        |. keyword "const"
        |= mspaces
...
equation = 
    succeed (\p spc1 params spc2 e1 spc3 -> \e2 ->
            case p of 
                -- Special handling for main. not modifiable.
                PVar _ "main" -> EApp ["EQ", spc1, spc2, spc3] (ELam [] p (EConst [] e2)) (getFuncDef params e1)
                _             -> EApp ["EQ", spc1, spc2, spc3] (ELam [] p e2) (getFuncDef params e1))
        |= pvar
        |= mspaces
        |= paramLoop
        |. symbol "="
...
glist : Parser Exp
glist =
    succeed (\spc1 e1 e2 spc2 -> EGList [spc1, spc2] e1 e2)
        |. symbol "{"
        |= mspaces
        |= lazy (\_ -> exp)
        |= gListLoop
