...
import Language.Fusion exposing (fuse)

feval : BEnv -> Exp -> Value
feval env exp =
    case exp of
        ETrue _     -> VTrue
        EFalse _    -> VFalse
            ...
            case feval env e1 of
                VClosure envf p ef ->
                    ...
                    case e2 of
                        EFix _ e ->
                            case p of
                                PVar _ s -> feval ((s, (VFix env e, DFix env e), BTrue)::envf) ef
                                _        -> VError "Error 46"

                        _ ->
                            case (feval env e2, BTrue) |> matchC p of
                                Just envm -> feval (envm ++ envf) ef
                                Nothing   -> VError "Error 10"

                _ -> let _ = Debug.log "Result:" (feval env e1) in
                     VError "Error 11"
        ...
        EGCons _  e1 e2 -> VCons  (feval env e1) (feval env e2)
        EList _   e1 e2 -> VCons  (feval env e1) (feval env e2)
        EGList _  e1 e2 -> VCons  (feval env e1) (feval env e2)
        ...
        EUnwrap _ e ->
            case feval env e of
                ...
                _ -> VError "Error 56"
        EConst _ e -> feval env e

        EDelta _ e delta -> let e1 = fuse delta e in feval env e1