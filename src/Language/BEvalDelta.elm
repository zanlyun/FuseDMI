module Language.BEvalDelta exposing (..)
...
bevalDelta : BEnv -> Exp -> Delta -> BST -> (BEnv, Exp, BST)
bevalDelta env exp delta st =
    case (exp, delta) of
        (_, DId)     -> (env, exp, st)
        (EConst wsc1 (EConst ws2 e), _) -> 
            let (new_env, new_e2, new_st) = bevalDelta env (EConst ws2 e) delta st
            in case new_e2 of 
        ...
        (EConst wsc e, DRewr p) ->
            ...
            then let pExp = param2EList p
                  ...
                  then let (new_env, new_e, new_st) = bevalDelta env e delta st in 
                       (new_env, EConst wsc new_e, new_st)
                  else ([], EError 0 "evaluates to const, not modifiable",[])
            else let (new_env, new_e, new_st) = bevalDelta env e delta st in 
            (new_env, EConst wsc new_e, new_st)
        (_, DRewr p) ->
        ...
        -- P-Constant2
        (EFloat ws f, DAdd (AFloat f1)) -> (env, EDelta ws (EFloat [" "] f) (DAddV (VFloat f1)), st)
        -- DMul is Not included 
        (EFloat ws f, DMul (AFloat f1)) -> (env, EFloat ws (f * f1), st)

        (ENil ws,     DInsert 0 p) -> (env, ECons [" "] (EDelta [" "] (param2Exp p) DInsertV) (ENil ws), st)
        (EEmpList ws, DInsert 0 p) -> (env, EList ws   (EDelta [" "] (param2Exp p) DInsertV) (EEmpList []), st)
        ...
        (EVar _ x, _) ->
            case lookup2 x env of
                Just ((v, od), bv) ->
                    if isConstBValue bv 
                    then (env, EDelta [" "] exp delta, st) -- P-Var2
        ...
        (EConst wsc (ELam ws p1 e1), _) -> 
          let (new_env, new_e, new_st) = bevalDelta env (ELam ws p1 (EConst [] e1)) delta st
          in case new_e of 
            ELam _ _ (EConst [] new_e1) -> (new_env, EConst wsc (ELam ws p1 new_e1),new_st)
            _ -> ([], EError 1 "fusion failed.",[])

        (EConst wsc (EApp ws e1 e2), _) -> 
            let (new_env, new_app, new_st) = bevalDelta env (EApp ws (EConst [] e1) (EConst [] e2)) delta st
            in case new_app of 
                        ...
                        Just envm ->
                            ...
                            in if isConstBValue b
                               then ([], EError 0 "evaluates to const, not modifiable.", [])
                                    ...
                                    then  -- P-App2
                                       let (env_, ef_, st_) = bevalDelta (envm ++ envf) ef delta st
                                           (envm_, envf_) = ( List.take (List.length envm) env_, List.drop (List.length envm) env_)
                                           (env1, new_e1, _) = bevalDelta env e1 (DClosure envf_ p ef_) []
                                        in if envm == envm_
                                       ...
                                       let (envm_, envf_) = (List.take (List.length envm) env_, List.drop (List.length envm) env_)
                                           (env1, new_e1, _) = bevalDelta env e1 (DClosure envf_ p ef_) []
                                           (env2, new_e2, _) = bevalDelta env e2 (substDelta p envm_) []
        ...
        (EFix ws1 e, DClosure env1 p1 e1) -> 
            let (new_env, new_e, new_st) = bevalDelta env e (DClosure env1 p1 e1) st
            in (new_env, EFix ws1 new_e, new_st)

        -- P-C-Case 
        (EConst wsc (ECase ws e0 bs), _) ->
            let (new_env, new_const_case, new_st) = bevalDelta env (ECase ws (EConst [] e0) (constlizeBranch bs)) delta st
        ...
        (ECase ws e0 bs, _) ->
            ...
            in case res.ei of
                ...
                _           ->
                    -- TODO: check if this is correct for ST
                    case bevalDelta (res.envm ++ env) res.ei delta st of
                        (env_, ei_, st_) ->
                            ...
                            in if isConstBValue b0 
                               then -- P-Case2
                                    ...
                                    let (env0, new_e0, _) = bevalDelta env e0 (substDelta res.pi envm_) [] -- from pattern to construct delta e0
        ...
        (EConst wsc (ECons ws e1 e2), DCons _ _) ->
            ...
            in case (b1, b2) of 
                  ...
                  _              -> 
                      let (new_env, e_new, st_new) = bevalDelta env (ECons ws (EConst [] e1) (EConst [] e2)) delta st             
                      in  case e_new of 
        ...
        (ECons ws e1 e2, DCons d1 d2) ->
            let
                (env1, new_e1, st1) = bevalDelta env e1 d1 st             
                (env2, new_e2, st2) = bevalDelta env e2 d2 st
        ...
        (ECons ws e1 e2, DInsert n p) ->
            if n == 0 
            then (env, ECons ws (EDelta [" "] (param2Exp p) DInsertV) exp, st)
            else let (env2, new_e2, st_) = bevalDelta env e2 (DInsert (n - 1) p) st
                in
                if isError new_e2
                then (env, fuse delta exp, st) -- back to trivial method
                else
        ...
        (ECons ws e1 e2, DDelete n) ->
            if n == 0 then (env, ECons ws (EDelta [" "] e1 DDeleteV) e2, st)
            else
                let (env2, new_e2, st_) = bevalDelta env e2 (DDelete (n - 1)) st
                in
                if isError new_e2
                then (env, fuse delta exp, st)
                else
        ...
        (ECons ws e1 e2, DCopy n) ->
            ...
            then (env, ECons ws e1 (ECons ws e1 e2), st)
                ...
                let (env2, new_e2, st_) = bevalDelta env e2 (DCopy (n - 1)) st
                in
                if isError new_e2
                then (env, fuse delta exp, st)
                else
        ...
        (ECons ws e1 e2, DModify n d) ->
            if n == 0 
            then
                let (env1, new_e1, st_) = bevalDelta env e1 d st
                in
                if isError new_e1
                then (env, fuse delta exp, st)
                else
                ...
                let (env2, new_e2, st_) = bevalDelta env e2 (DModify (n - 1) d) st
                in
                if isError new_e2
                then (env, fuse delta exp, st)
                else
        ...
        (ECons ws e1 e2, DMem s (ACons a1 a2)) ->
            let (env_, e2_, st_) = bevalDelta env e2 (DMem s a2) st  -- using a2 to update e2, Mem means cache in state.
            in
            case a1 of
        ...
        (EConst wsc (EGCons ws e1 e2), DCons _ _) ->
            ...
            in case (b1, b2) of 
                  (BTrue, BTrue) -> ([], EError 0 "e1, e2 evaluates to const, not modifiable.", [])
                  _              -> 
                      let (new_env, e_new, st_new) = bevalDelta env (EGCons ws (EConst [] e1) (EConst [] e2)) delta st             
                      in  case e_new of 
        ...
        (EGCons ws e1 e2, DInsert n p) ->
            if n == 0 
            then (env, EGCons ws (EDelta [" "] (param2Exp p) DInsertV) exp, st)
            else let (env2, new_e2, st_) = bevalDelta env e2 (DInsert (n - 1) p) st
                in
                if isError new_e2
                then ([], EError 1 "fuse failed, EGCons not modifiable.", [])
        ...
        (EGCons ws e1 e2, DDelete n) ->
            if n == 0 then (env, EGCons ws (EDelta [" "] e1 DDeleteV) e2, st)
            else
                let (env2, new_e2, st_) = bevalDelta env e2 (DDelete (n - 1)) st
                in
                if isError new_e2
        ...
        (EConst wsc (EList ws e1 e2), DCons _ _) ->
            ...
            in case (b1, b2) of 
                  (BTrue, BTrue) -> ([], EError 0 "e1, e2 evaluates to const, not modifiable.", [])
                  _              -> 
                      let (new_env, e_new, st_new) = bevalDelta env (EList ws (EConst [] e1) (EConst [] e2)) delta st             
                      in  case e_new of 
                          EList _ (EConst [] e1_new) (EConst [] e2_new) -> (new_env, EConst wsc (EList ws e1_new e2_new), st_new)
                          _                                             -> ([], EError 1 "Fusion failed.", [])
        (EList ws e1 e2, DCons d1 d2) ->
            let (env1, new_e1, st1) = bevalDelta env e1 d1 st
                (env2, new_e2, st2) = bevalDelta env e2 d2 st
                new_ST = mergeST st1 st2
            in
            if isError new_e1 || isError new_e2
        ...
        (EList ws e1 e2, DInsert n p) ->
            if n == 0 
            then
                (env, EList ws (EDelta [" "] (param2Exp p) DInsertV) exp, st)
            else
                let
                    (env2, new_e2, st_) = bevalDelta env e2 (DInsert (n - 1) p) st
                in
                if isError new_e2
                then
                    (env, fuse delta exp, st)
        ...
        (EList ws e1 e2, DDelete n) -> 
            if n == 0 
            then
                (env, EList ws (EDelta [" "] e1 DDeleteV) e2, st)
                -- case ws of
                --     _::[] -> (env, EList ws (EDelta [] e1 DDeleteV) e2, st)
                --     _     ->
                --         case e2 of
                ...
                let
                    (env2, new_e2, st_) = bevalDelta env e2 (DDelete (n - 1)) st
                in
                if isError new_e2 
                then
                    (env, fuse delta exp, st)
        ...
        (EList ws e1 e2, DCopy n) ->
            ...
            then
                ...
                let
                    (env2, new_e2, st_) = bevalDelta env e2 (DCopy (n - 1)) st
                in
                if isError new_e2
                then
                    (env, fuse delta exp, st)
        ...
        (EList ws e1 e2, DModify n d) ->
            ...
            then
                let
                    (env1, new_e1, st_) = bevalDelta env e1 d st
                in
                if isError new_e1 
                then (env, fuse delta exp, st)
                else
                ...
                let
                    (env2, new_e2, st_) = bevalDelta env e2 (DModify (n - 1) d) st
                in
                if isError new_e2 
                then (env, fuse delta exp, st)
                else
        ...
        (EList ws e1 e2, DMem s (ACons a1 a2)) ->
            let
                (env_, e2_, st_) =
                    bevalDelta env e2 (DMem s a2) st
            in
            case a1 of
                ATrue -> case lookup s st_ of
        ...
        (EConst wsc (EGList ws e1 e2), DCons _ _) ->
            ...
            in case (b1, b2) of 
                  (BTrue, BTrue) -> ([], EError 0 "e1, e2 evaluates to const, not modifiable.", [])
                  _              -> 
                      let (new_env, e_new, st_new) = bevalDelta env (EGList ws (EConst [] e1) (EConst [] e2)) delta st             
                      in  case e_new of 
        ...
        (EGList ws e1 e2, DInsert n p) ->
            if n == 0 
            then
                (env, EGList ws (EDelta [" "] (param2Exp p) DInsertV) exp, st)
            else
                let
                    (env2, new_e2, st_) = bevalDelta env e2 (DInsert (n - 1) p) st
                in
        ...
        (EGList ws e1 e2, DDelete n) ->
            if n == 0 
            then
                (env, EGList ws (EDelta [" "] e1 DDeleteV) e2, st)
                -- case ws of
                --     _::[] -> (env, e2, st)
                --     _     ->
                --         case e2 of
        ...
        (EConst wsc (ETuple ws e1 e2), DTuple _ _) ->
            ...
            in case (b1, b2) of 
                  (BTrue, BTrue) -> ([], EError 0 "e1, e2 evaluates to const, not modifiable.", [])
                  _              -> 
                      let (new_env, e_new, st_new) = bevalDelta env (ETuple ws (EConst [] e1) (EConst [] e2)) delta st             
                      in case e_new of 
        ...
        (EConst wsc (EParens ws e1), _) ->
            let (env1, new_e1, st_) = bevalDelta env (EConst [] e1) delta st
            in
            if isError new_e1
        ...
        (EParens ws e1, _) ->
            let
                (env1, new_e1, st_) = bevalDelta env e1 delta st
            in
            if isError new_e1
            then (env,  EError 1 "Fusion Failed", st_)
        ...
        (EConst wsc (EBPrim ws Add e1 e2), _) ->
             case (fevalC env (EConst [] e1), fevalC env (EConst [] e2)) of 
                  ...
                  _              -> 
                      let (new_env, new_e, new_st) = bevalDelta env (EBPrim ws Add (EConst [] e1) (EConst [] e2)) delta st
                      in case new_e of 
        ...
        (EBPrim ws Add e1 e2, DAdd _) ->
            ...
            in case (b1, b2) of 
                ...
                (BTrue, BFalse) ->  -- P-Oplus2
                   let (env2, new_e2, _) = bevalDelta env e2 delta []
                       (fv1, fv2) = (freeVars e1, freeVars e2)
                ...
                _               -> -- P-Oplus3
                   let (env1, new_e1, _) = bevalDelta env e1 delta []
                       (fv1, fv2) = (freeVars e1, freeVars e2)
                    ...
                    case (b1, b2) of 
                        ...
                        (BTrue, BFalse) ->  -- P-Oplus-2
                            let delta2 = ((f1 + f2) * f - f1) / f2 |> AFloat |> DMul
                                (env2, new_e2, _) = bevalDelta env e2 delta2 []
                                (fv1, fv2) = (freeVars e1, freeVars e2)
                            in
                        ...
                        _ -> -- P-Oplus3
                            let delta1 = ((f1 + f2) * f - f2) / f1 |> AFloat |> DMul
                                (env1, new_e1, _) = bevalDelta env e1 delta1 []
                                (fv1, fv2) = (freeVars e1, freeVars e2)
                            in
            ...
            case (fevalC env (EConst [] e1), fevalC env (EConst [] e2)) of
                ((VFloat _, b1), (VFloat _, b2)) ->
                    case (b1, b2) of 
                        (BTrue, BTrue)  -> ([], EError 0 "e1, e2 evaluates to const, not modifiable.", [])
                        _               ->
                          let (new_env, new_e, new_st) = bevalDelta env (EBPrim ws Sub (EConst [] e1) (EConst [] e2)) delta st
                          in case new_e of 
            ...
            case (fevalC env (EConst [] e1), fevalC env (EConst [] e2)) of
                ((VFloat _, b1), (VFloat _, b2)) ->
                    case (b1, b2) of 
                        (BTrue, BTrue)  -> ([], EError 0 "e1, e2 evaluates to const, not modifiable.", [])
                        _               ->
                          let (new_env, new_e, new_st) = bevalDelta env (EBPrim ws Mul (EConst [] e1) (EConst [] e2)) delta st
                          in case new_e of 
        ...
        (EBPrim ws Mul e1 e2, DAdd (AFloat f)) ->
            ...
            in case (b1, b2) of 
                ...
                (BTrue, BFalse)  -> -- P-Oplus2
                    case v1 of
                        VFloat f1 ->
                            let (env2, new_e2, _) = bevalDelta env e2 ((f / f1 |> AFloat) |> DAdd) []
                                (fv1, fv2) = (freeVars e1, freeVars e2)
                            in
                            case two_wayMerge fv1 fv2 env env2 of
                ...
                _                ->  -- P-Oplus3
                    case v2 of
                        VFloat f2 ->
                            let (env1, new_e1, _) = bevalDelta env e1 ((f / f2 |> AFloat) |> DAdd) []
                                (fv1, fv2) = (freeVars e1, freeVars e2)
                            in
                            case two_wayMerge fv1 fv2 env1 env of
        ...
        (EConst wsc (EUPrim ws Neg e1), _) -> 
           let (new_env, new_e, _) = bevalDelta env (EUPrim ws Neg (EConst [] e1)) delta st
           in case new_e of 
        ...
        (EUPrim ws Neg e1, DAdd (AFloat f)) ->
            ...
            in case b1 of 
                ...
                _ -> let (env1, new_e1, _) = bevalDelta env e1 (AFloat -f |> DAdd) []
        ...
        (EUPrim ws Neg e1, DMul _) ->
            ...
            in case b1 of 
                BTrue -> ([], EError 0 "e1 evaluates to const, not modifiable.", [])
                _     -> let (env1, new_e1, _) = bevalDelta env e1 delta []
                         in (env1, EUPrim ws Neg new_e1, st)

        -- P-C-Unwrap
        (EConst wsc (EUnwrap ws e), _) -> 
            let (env1, new_const_e, st_) = bevalDelta env (EConst [] e) (DMap delta) st
        ...
        -- P-Unwrap
        (EUnwrap ws e,  _) ->
            let (env1, new_e, st_) = bevalDelta env e (DMap delta) st
                ...
                (VClosure envf p ef, _) ->
                    case fevalC env e2 of
                        (VGraphic _ v, bv) ->
                            ...
                            Just envm ->
                                let (env_, ef_, st_) = bevalDelta (envm ++ envf) ef d st
                                    (envm_, envf_) = (List.take (List.length envm) env_, List.drop (List.length envm) env_)
                                    (env1, new_e1, _) = bevalDelta env e1 (DClosure envf_ p ef_) []
                                    (env2, new_e2, _) = bevalDelta env e2 (substDelta p envm_ |> DMap) []
                                    (fv1, fv2) = (freeVars e1, freeVars e2)
                                in
                                if isError new_e1 || isError new_e2
                                then (env, fuse delta exp, st)
        ...
        (EConst wsc (EGraphic ws s e), _) -> 
            let (new_env, new_g_e, new_st) = bevalDelta env (EGraphic ws s  (EConst [] e)) delta st
            in case new_g_e of 
        ...
        (EGraphic ws s e, DMap d) ->
            ...
            in if isConstBValue b  
               ...
               else let (env1, new_e, st_) = bevalDelta env e d st
        ...
        (e, DCtt (PVar _ s) d) ->
            let (env_, e_, st1) = bevalDelta env e d ((s, (env, EError 2 "IIIegal Constraint"))::st)
            in
            if isError e_ 
            then (env, fuse delta exp, st)
            else
        ...
        (e, DGroup s d) ->
            let (env_, e_, st1) = bevalDelta env e d ((s, ([], EEmpList []))::st)
            in case e_ of 
                EError _ _ -> ([], e_, [])
                _        ->
                    case st1 of
        ...
        (e, DCom d1 d2) ->
            let (env1, e1, st1) = bevalDelta env e d1 st
            in if isError e1
            then (env, fuse delta exp, st)
            else let (env2, e2, st2) = bevalDelta env1 e1 d2 st
            in if isError e2
            then (env, fuse delta exp, st)
