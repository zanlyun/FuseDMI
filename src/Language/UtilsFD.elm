...
dmatch p param =
    case (p, param) of
        ...
        (PCons _ p1 p2, AGCons a1 a2) ->
            case (dmatch p1 a1, dmatch p2 a2) of
                (Just ctx1, Just ctx2) -> Just (ctx1 ++ ctx2)
                _                      -> Nothing

        (PList _ p1 p2, AGCons a1 a2) ->
            case (dmatch p1 a1, dmatch p2 a2) of
                (Just ctx1, Just ctx2) -> Just (ctx1 ++ ctx2)
                _                      -> Nothing
...
param2EList param =
    case param of
        ...
        AGCons a1 a2 -> EGList [" ", ""] (param2Exp a1) (param2EList a2)
        _           ->  EError 2 "Error 06"
...
mergeST st1 st2 =
    case (st1, st2) of
        ((s1, (env1, e1))::st1_, (_, (_, EError _ _))::st2_) ->
            (s1, (env1, e1))::mergeST st1_ st2_

        ((s1, (env1, EError _ _))::st1_, (_, (_, e2))::st2_) ->
...
updateST : BST -> String -> Exp -> BST
updateST st s e =
    case st of
        [] -> []
        (s1, (env1, e1)) :: st_ ->
            -- update the binding expression of var s to e
            if s == s1 then
                (s1, (env1, e))  :: st_
            else
...
fixCheck v d =
    case (v, d) of
        (VFix _ _, DClosure ((_, (_, DFix _ (ELam ws1 p1 (ELam ws2 p2 e1))), _) :: env2) _ e2) ->
            if e1 == e2 then
        ...
        -- TODO: don't know why
        (VFix _ _, DClosure ((_, (_, dfix), _) :: env2) _ e2) -> Just dfix
        _ -> 
          let
              _ = Debug.log "fixCheck v" v
              _ = Debug.log "fixCheck d" d
          in Just d