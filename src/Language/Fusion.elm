...
import Language.Utils exposing (varsInPattern, isError, isBranchError)
import Language.UtilsFD exposing (param2Exp, eqDelta)

fuse : Delta -> Exp -> Exp
fuse delta exp =
    if checkDeltaToFuse delta True |> not then
        EError 1 "fuse, Fusion Failed"
    else
        fuse_ delta exp
...
fuse_ delta exp = 
  let 
    _ = Debug.log "fuse_ delta" delta
    _ = Debug.log "fuse_ exp" exp
  in
  case exp of 
    EConst _ _ -> case delta of 
        DId -> exp
        _   -> EError 0 "fuse_, const marked expression is not modifiable."
    EGList ws e1 e2 -> case delta of 
        DId -> exp
        DCons d1 d2 ->
          let e1_ = fuse_ d1 e1
          in case e1_ of 
              EError _ _ -> e1_
              _ -> let e2_ = fuse_ d2 e2
                   in case e2_ of 
                       EError _ _ -> e2_
                       _          -> EGList ws e1_ e2_
        _   -> EError 0 "fuse_ EGList, expression structure is not modifiable."
    EGCons ws e1 e2 -> case delta of 
        DId -> exp
        DCons d1 d2 ->
          let e1_ = fuse_ d1 e1
          in case e1_ of 
              ...
              _ -> let e2_ = fuse_ d2 e2
                   in case e2_ of 
                       EError _ _ -> e2_
                       _          -> EGCons ws e1_ e2_
        _   -> EError 0 "fuse_ EGCons, expression structure is not modifiable."
    ...
    _ -> case delta of
            ...
            case p of
                PVar _ var ->
                    case getCttParamPath d (AVar var) [] of
                        ...
                        Nothing   -> EError 2 "Error 57"
                
                _   -> EError 2 "Error 58"
        
        DAdd p ->
            case exp of 
                _          -> let spc = getLastSpaces exp
                              in EBPrim [""] Add (pars "" exp |> setLastSpaces "") (param2Exp p) |> pars spc
        DMul p ->
            case exp of 
                _          -> let spc = getLastSpaces exp
                              in EBPrim [""] Mul (pars "" exp) (param2Exp p) |> pars spc
        ...
        DCom d1 d2 ->
            let exp1 = fuse_ d1 exp
            in case exp1 of 
                EError _ _ -> exp1
                _          -> fuse_ d2 exp1
        
        _ -> EError 1 "254 fuse_, Fusion Failed"
...
genFunByPath path =
    case path of
        ...
        _                     -> EError 2 "genFunByPath Failed"
...
fuseVar delta var exp =
    case exp of
        ...
        ELam _ p e ->
            if not (List.member var (varsInPattern p)) 
            then
                let e_ = fuseVar delta var e 
                in case e_ of 
                    EError _ _ -> e_
                    _          -> ELam [] p e_
            else exp
        ...
        EApp ws e1 e2  -> 
            ...
            in if isError e1_ || isError e2_
            then EError 0 "fuseVar EApp fail."
            else EApp ws e1_ e2_
        
        ECase ws e bs  ->
            let
                e_ = fuseVar delta var e
                bs_ = fuseVarBranch delta var bs
            in if isError e_ || isBranchError bs_
            then EError 0 "fuseVar ECase fail."
            else ECase ws e_ bs_
        ...
        EBPrim ws bop e1 e2 -> 
            let 
                ...
                e2_ = fuseVar delta var e2
            in if isError e1_ || isError e2_
            then EError 0 "fuseVar EBPrim fail."
            else EBPrim ws bop e1_ e2_
        ...
        ECons ws e1 e2      -> 
            let
                ...
                e2_ = fuseVar delta var e2
            in if isError e1_ || isError e2_ 
            then EError 0 "fuseVar ECons fail."
            else ECons ws e1_ e2_
        ...
        EList ws e1 e2      -> 
            let
                ...
                e2_ = fuseVar delta var e2
            in if isError e1_ || isError e2_ 
            then EError 0 "fuseVar EList fail."
            else EList ws e1_ e2_
        ...
        ETuple ws e1 e2     -> 
            let
                ...
                e2_ = fuseVar delta var e2
            in if isError e1_ || isError e2_ 
            then EError 0 "fuseVar ETuple fail."
            else ETuple ws e1_ e2_
        
        EUPrim ws uop e -> 
            let
                e_ = fuseVar delta var e
            in if isError e_ 
            then EError 0 "fuseVar EUPrim fail."
            else EUPrim ws uop e_

        EParens ws  e   -> 
            let
                e_ = fuseVar delta var e
            in if isError e_
            then EError 0 "fuseVar EParens fail."
            else EParens ws e_
        ...
        EFix ws     e   -> 
            let
                e_ = fuseVar delta var e
            in if isError e_
            then EError 0 "fuseVar EFix fail."
            else EFix ws e_
        ...
        EGraphic ws s e -> 
            ...
            in
              if isError e_
              then EError 0 "fuseVar EGraphic fail."
              else EGraphic ws s e_

        EMap ws f e ->
            let
                f_ = fuseVar delta var f
                e_ = fuseVar delta var e
            in if isError f_ || isError e_ 
            then EError 0 "fuseVar EMap fail."
            else EMap ws f_ e_
        ...
        EUnwrap ws e ->
            let
                e_ = fuseVar delta var e
            in if isError e_
            then EError 0 "fuseVar EUnwap fail."
            else EUnwrap ws e_
        EConst _ _ -> EError 1 "fuseVar fail, expression not modifiable, const marked."
        EGList ws e1 e2 -> 
            let
                e1_ = fuseVar delta var e1
                e2_ = fuseVar delta var e2
            in
              if isError e1_ || isError e2_
              then EError 0 "fuseVar EGList fail."
              else EGList ws e1_ e2_
        EGCons ws e1 e2 -> 
            let
                e1_ = fuseVar delta var e1
                e2_ = fuseVar delta var e2
            in
              if isError e1_ || isError e2_
              then EError 0 "fuseVar EGCons fail."
              else EGCons ws e1_ e2_
 
        -- EError 1 "fuseVar fail, EGList not modifiable."
        _ -> exp
...
fuseEnv deltas exp =
    case deltas of
        ...
        (var, delta) :: rest ->
            if checkDeltaToFuse delta True |> not then
                EError 1 "fuseEnv, Fusion Failed"
            else
            let
                exp1 = fuseEnv rest exp
            in  case exp1 of 
                  EError _ _ -> exp1
                  _ -> fuseVar delta var exp1
